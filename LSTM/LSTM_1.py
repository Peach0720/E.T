...
def LSTM(code):
    ...
    query = sqlalchemy.select(table).where(table.c.Ticker == code)  # 해당 종목 쿼리
    print(query)
    ...
    df = df[['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Ticker']].copy()
    ...
    # 데이터를 로딩한다.
    raw_dataframe = df[['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume']].copy()
    raw_dataframe.info()  # 데이터 정보 출력
    print(raw_dataframe)
    ##임의로 추가함 마지막 정답 확인
    real_price = raw_dataframe.iloc[-1].copy()
    real_price = real_price[['Close','Date']].copy()
    #raw_dataframe = raw_dataframe.iloc[:-1, :].copy()

    # raw_dataframe.drop('Date', axis=1, inplace=True) # 시간열을 제거하고 dataframe 재생성하지 않기
    del raw_dataframe['Date']   # 위 줄과 같은 효과

    stock_info = raw_dataframe.values[1:].astype(np.float32)  # 금액&거래량 문자열을 부동소수점형으로 변환한다
    ...
    multi_cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell(stackedRNNs,
    ...
    hypothesis = tf.compat.v1.layers.dense(hypothesis[:, -1], output_data_column_cnt, activation=tf.identity) #한번 고침

    # 손실함수로 평균제곱오차를 사용한다
    loss = tf.reduce_sum(input_tensor=tf.square(hypothesis - Y))
    ...
    rmse = tf.sqrt(tf.reduce_mean(input_tensor=tf.math.squared_difference(targets, predictions)))
    ...
    print(real_price['Date'],", real price ",real_price['Close']) #임의로 추가한 정답


LSTM('035420')
['005930' '000660' '035420' '005380' '035720' '005490' '066570']